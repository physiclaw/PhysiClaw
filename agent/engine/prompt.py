...
def _render_examples() -> list[str]:
    ...
    return [
        ...
        "",
        "**Text rows survive compaction; icon rows don't.** Earlier views are stubbed to header + text-kind rows (no image, no icon rows). A labelled target you've seen before is still reachable via its surviving text row many turns later; an unlabelled icon isn't.",
        "❌ Wrong: tapping a bbox that came from an earlier view's IMAGE (now dropped). If the bbox wasn't a text row, it's gone.",
        "✅ Right: for nav tabs, named CTAs (加入购物车, 去结算), category labels, product titles — reference the surviving text row. For unlabelled icons (app-icon grid thumbnails, icon-only buttons), re-peek when you need them.",
        "",
