...
def _render_examples() -> list[str]:
    ...
    return [
        ...
        "",
        "**`scan` verifies; `peek` plans.** Never pick a tap target from a `scan` alone — text-only listings can match the wrong element. Use `peek` to ground the bbox; if `peek` doesn't list the target, escalate to `screenshot`.",
        "❌ Wrong: scan finds a row labeled `JD` → tap it. (You don't know if it's the JD app icon or a `JD` link on a Safari page.)",
        "✅ Right: `peek` first to see the visual context and grounded bbox; tap from that listing. After the tap, `scan` to verify the screen changed.",
        "",
