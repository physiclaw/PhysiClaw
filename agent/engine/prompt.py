...
def _render_examples() -> list[str]:
    ...
    return [
        ...
        "",
        "**Escalate view tools when scan misses your target.** Scan is text-only — if it keeps returning the same listing, you can't tell whether you're on the wrong page or the target element is too small for it to catch. Switch to `peek` (visual) or `screenshot` (pixel-perfect).",
        "❌ Wrong: scan finds no JD icon → scan again → scan again → tap some random text labelled \"JD\" in the listing.",
        "✅ Right: scan finds no JD icon → next turn `peek` to see what page you're actually on. If peek still doesn't list it, next turn `screenshot`.",
        "",
