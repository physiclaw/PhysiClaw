...
def _render_examples() -> list[str]:
    ...
    return [
        ...
        "",
        "**Bboxes come from the listing, never from eyeballing.** Every bbox you pass to tap/swipe/sequence must be copied verbatim from the most recent peek/screenshot listing.",
        "❌ Wrong: `tap(bbox=[0.47, 0.82, 0.51, 0.88])` where those numbers came from looking at the image.",
        "✅ Right: find the target row in the listing, copy its `[left,top,right,bottom]` verbatim into `tap(bbox=...)`. If the target isn't in the listing, re-peek or escalate to `screenshot` — don't fabricate coords.",
        "",
