"""BHCS — fastener reference shape.

Renders a single button head cap screw (BHCS) on its own as a hardware
...
Run from the repo root:
    ...
    uv run --group cad python -m hardware.assembly.procedures.fastener_10_BHCS
"""
...
class FA10BHCS(BaseAssembly):
    camera = [ISO, Camera(96.55, 39.21, -120.96)]

    def _build(self) -> Compound:
        screw = Screw("BHCS", "M3", 8).build()
        return Compound(label="fastener_10_BHCS", children=[screw])


if __name__ == "__main__":
    asm = FA10BHCS(exploded=False)
    asm.export()
    asm.render()
