"""SHCS — fastener reference shape.

Renders a single socket head cap screw (SHCS) on its own as a hardware
...
Run from the repo root:
    ...
    uv run --group cad python -m hardware.assembly.procedures.fastener_11_SHCS
"""
...
class FA11SHCS(BaseAssembly):
    camera = [ISO, Camera(96.55, 39.21, -120.96)]

    def _build(self) -> Compound:
        screw = Screw("SHCS", "M3", 10).build()
        return Compound(label="fastener_11_SHCS", children=[screw])


if __name__ == "__main__":
    asm = FA11SHCS(exploded=False)
    asm.export()
    asm.render()
