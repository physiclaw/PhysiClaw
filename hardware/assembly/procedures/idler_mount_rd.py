...
Run from the repo root:
    ...
    uv run --group cad python -m hardware.assembly.procedures.idler_mount_rd
"""
...
class IdlerMountRd(BaseAssembly):
    camera = Camera(-30, 25)
    ...
    def _build(self) -> Compound:
        ...
        return Compound(label="idler_mount_rd", children=[block, *placed])


if __name__ == "__main__":
    for exploded in (True, False):
        asm = IdlerMountRd(exploded=exploded)
        asm.export()
        asm.render()
