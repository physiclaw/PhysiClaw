...
def _render_examples() -> list[str]:
    ...
    return [
        ...
        "",
        "**Write data to scratchpad before leaving the page where you read it.** When the next tool will change the screen (`home_screen`, `go_back`, a `tap` that opens elsewhere) and the data on the current page is something the next page won't show — pass `scratchpad=...` on this turn's `note`.",
        "❌ Wrong: read 6 JD orders by scrolling → `[note(summary=\"Captured 6 orders, going to WeChat\"), home_screen()]`. The list is in tool_results that get stubbed by the next `peek`; you'll re-open JD to re-verify.",
        "✅ Right: `[note(summary=\"Captured 6 orders, going to WeChat\", scratchpad=\"1. cable ¥45 (delivered)\\n2. clips ¥17 (shipped)\\n3. ...\"), home_screen()]`. The list is now pinned in `<scratchpad>` and outlives every later `peek` and any compaction. Compose the WeChat reply directly from it.",
        "",
