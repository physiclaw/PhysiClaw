...
from physiclaw.conductor.spec import memory, reply
...
def check_boot(nodes: list[Node]) -> None:
    ...
    grammar."""
    activates = [n.id for n in nodes if isinstance(n, SelectNode)]
    if not nodes or not isinstance(nodes[-1], SelectNode):
        raise PlaybookError(
...
def pack_warnings(pack: Pack, specs: Iterable[Playbook]) -> list[str]:
    ...
    read: set[str] = set().union(*(spec.prompts_used for spec in specs))
    out += [
...
def _prompt_warnings(spec: Playbook) -> list[str]:
    ...
    for n in flatten(spec.nodes):
        ...
        out += [
            ...
            if name not in memory.PARTS and name.casefold() not in low
        ]
        # The sharp one: `never_tap:` is withheld from the model BY
        # DESIGN (`spec.fence.refusal`), so a prompt that repeats a
