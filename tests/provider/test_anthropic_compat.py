"""Tests for `physiclaw.provider.anthropic_compat`.
...
from physiclaw.provider.anthropic_compat import (
    _DEFAULT_MAX_TOKENS,
    _STOP_REASON_MAP,
...
def test_encode_unknown_message_type_returns_none_with_warning(
    ...
    with caplog.at_level(logging.WARNING, logger="physiclaw.provider.anthropic_compat"):
...
async def test_chat_maps_4xx_status_to_permanent(
    ...
    with caplog.at_level(logging.ERROR, logger="physiclaw.provider.anthropic_compat"):
...
def test_content_to_anthropic_drops_unknown_block_with_warning(
    ...
    with caplog.at_level(logging.WARNING, logger="physiclaw.provider.wire"):
...
def test_assistant_blocks_generates_id_when_tool_call_id_empty(mocker) -> None:
    mocker.patch(
        "physiclaw.provider.anthropic_compat.uuid.uuid4",
